...
# 使用FlashDepth模型
from flashdepth_wrapper import create_flashdepth_model
...
def logging_setup():
    ...
    # 配置日志 - 强制刷新缓冲区
    file_handler = logging.FileHandler('logs/pano2stereo_flashdepth.log', mode='w', encoding='utf-8')
    ...
    # 强制刷新
    logging.info("=== Pano2Stereo with FlashDepth Processing Started ===")
...
class FlashDepthStereoPano():
    def __init__(self, height, width, IPD, pro_radius, pixel, critical_depth, vit_size='vitl', dataset='hypersim'):
        logging.info('Initializing FlashDepth StereoPano Generator')
        self.IPD = IPD
        self.width = width
        self.height = height
        ...
        # 立即加载FlashDepth模型
        logging.info("Loading FlashDepth model...")
        # 根据vit_size选择FlashDepth模型类型
        flashdepth_model_map = {
            'vits': 'flashdepth-s',
            'vitb': 'flashdepth',      # 使用标准版本
            'vitl': 'flashdepth-l'
        }
        
        flashdepth_model_type = flashdepth_model_map.get(vit_size, 'flashdepth-l')
        
        try:
            self.model = create_flashdepth_model(
                model_type=flashdepth_model_type,
                device=self.device
            )
            logging.info(f'[FlashDepth] Model loaded: {flashdepth_model_type}')
        except Exception as e:
            logging.error(f"Failed to load FlashDepth model: {e}")
            raise RuntimeError(f"FlashDepth model initialization failed: {e}")
        ...
        # 模型预热（在初始化时完成）
        logging.info("Warming up FlashDepth model for optimal performance...")
        # 使用实际输入尺寸进行预热
        dummy_input = np.random.rand(height, width, 3).astype(np.uint8)
        
        try:
            _ = self.model.infer_image(dummy_input)
            logging.info("FlashDepth model warmup completed")
        except Exception as e:
            logging.warning(f"FlashDepth warmup failed: {e}")
        
        # 清理预热时的内存
        if torch.cuda.is_available():
            torch.cuda.empty_cache()
        
        logging.info('[FlashDepthStereoPano] Initialization completed')
    ...
    def make_output_dir(self, base_dir=None):
        ...
        index = 0
        while True:
            target_dir = f"{base_dir}_fd_{index:03d}" if index >= 0 else base_dir
...
def estimate_depth_flashdepth(generator, rgb_array):
    """使用FlashDepth模型进行深度估计"""
...
def main_flashdepth():
    ...
    logging.info(f"\n{'='*60}")
    logging.info(f"🚀 FlashDepth Core Processing Performance Analysis")
    logging.info(f"{'='*60}")
    ...
    try:
        ...
        logging.info(f'[FlashDepth Stereo] IPD: {IPD}m, Critical Depth: {CRITICAL_DEPTH:.2f}m')
        # 强制刷新日志
        for handler in logging.getLogger().handlers:
            handler.flush()
            
        generator = FlashDepthStereoPano(
        ...
        # 3. 🧠 深度估计（核心计时）
        depth_gpu = estimate_depth_flashdepth(generator, rgb_array)
        ...
        logging.info(f"\n🏆 FlashDepth Core Processing Summary:")
        ...
        logging.info(f"  🚀 Potential FPS: {1/total_core_time:.1f}")
        ...
        logging.info("=== FlashDepth Processing completed successfully ===")
        # 最终刷新
        for handler in logging.getLogger().handlers:
            handler.flush()
            
    except Exception as e:
        logging.error(f"FlashDepth processing failed: {str(e)}")
        raise
